...
async def process_repositories(repositories: typing.List[Repository], log: LoggerAdapter) -> None:
    log.info(f"Обработка {len(repositories)} репозиториев")

    for repo in repositories[:5]:
        log.info(f"Репозиторий: {repo.owner}/{repo.name}")
        log.info(f"  Позиция: {repo.position}")
        log.info(f"  Звезды: {repo.stars}")
        log.info(f"  Язык: {repo.language}")
        log.info(f"  Количество авторов с коммитами за последний день: {len(repo.authors_commits_num_today)}")
        ...
        for author in top_authors:
            log.info(f"    {author.author}: {author.commits_num} коммитов")
...
async def main() -> None:
    ...
    try:
        ...
        async with aiohttp.AsyncExitStack() as stack:
            ...
            log.info("Получение списка репозиториев...")
